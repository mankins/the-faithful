...
exports.process = async (params, isInline) => {
  ...
  try {
    ...
    ctx.font = `${fitTextOnCanvas(ctx, bigText, 'recoleta-bold', w-40)}px recoleta-bold`;
    ctx.fillStyle = bigStyle || bgStyle;
    ctx.fillText(bigText, 20, 85 + offset);
...
//    var textLeft = 0; // = parseInt(params.textleft, 10) || ((w / 2) - parseInt(m.width / 2, 10));
    ...
    ctx.fillText(line, textLeft, 157 + offset);
    ...
    ctx.fillText(line, textLeft, 195 + offset);
