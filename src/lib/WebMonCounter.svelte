<script>
  ...
  onMount(() => {
    if (window.document.monetization) {
      visible = true;
      window.document.monetization.addEventListener(
        'monetizationprogress',
        function (ev) {
          if ((total += Number(ev.detail.amount))) {
            let scale = 6;
            if (
              ('XRP' !== ev.detail.assetCode &&
                'USD' !== ev.detail.assetCode) ||
              9 !== ev.detail.assetScale
            ) {
              scale = ev.detail.assetScale;
            }

            (function updateAmount(tick) {
              // console.log({ tickTotal, total, a: ev.detail.amount });
              if (details && (tickTotal < total)) {
                const tick = Math.ceil((total - tickTotal) / 10);
                totalDisplay = (
                  (tickTotal + tick) /
                  Math.pow(10, assetScale)
                ).toFixed(scale);
                tickTotal = tickTotal + tick;
...
{#if visible}
  <button
    ...
    on:click={toggle}
    class="z-40 text-gray-500 focus:outline-none opacity-50 flex group-hover:opacity-100"
  >
    <span class="sr-only">Web Monetization Transfer</span>
    <svg
      class="h-6 w-6 mr-2"
      ...
      ><g id="Layer_2" data-name="Layer 2"
        ><g id="Coil_icon_black_and_white" data-name="Coil icon black and white"
    ...
    {#if details}
      ...
      transition:fade
        ...
        ><strong>{assetCode}</strong> {totalDisplay}</span
      >
    {/if}
  </button>
{/if}
